...
func main() {
	  ...
	  tries := flag.Int("t", 3, "How many tries do you want to allow?")
		flag.Parse()
		maxNum := 10
		if *tries > 5 {
			maxNum = 100
    ...
    fmt.Println("Game: Guess a number between 0 and",maxNum)
    ...
    secretNumber := randomizer.Intn(maxNum+1)  // generate numbers between 0 and 10 only
